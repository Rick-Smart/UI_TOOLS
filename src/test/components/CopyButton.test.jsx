...
describe("CopyButton", () => {
  it("renders a button with default label 'Copy summary'", () => {
    renderWithRouter(<CopyButton getSummary={() => "text"} />);
    expect(
  ...
  it("renders a custom label via children", () => {
    renderWithRouter(<CopyButton getSummary={() => "text"}>Copy my thing</CopyButton>);
    expect(
  ...
  it("shows 'Copied.' after successful copy", async () => {
    ...
    renderWithRouter(<CopyButton getSummary={() => "some summary"} />);
  ...
  it("calls getSummary on click", async () => {
    ...
    const user = userEvent.setup();
    renderWithRouter(<CopyButton getSummary={getSummary} />);
    await user.click(screen.getByRole("button"));
  ...
  it("does not call clipboard if getSummary returns empty string", async () => {
    ...
    const user = userEvent.setup();
    renderWithRouter(<CopyButton getSummary={() => ""} />);
    await user.click(screen.getByRole("button"));
  ...
  it("does not call clipboard if getSummary is not a function", async () => {
    navigator.clipboard.writeText = vi.fn();
    const user = userEvent.setup();
    renderWithRouter(<CopyButton getSummary={null} />);
    await user.click(screen.getByRole("button"));
    expect(navigator.clipboard.writeText).not.toHaveBeenCalled();
  });

  it("disabled prop is forwarded — button is disabled", () => {
    renderWithRouter(<CopyButton getSummary={() => "x"} disabled />);
  ...
  it("status text has muted class", async () => {
    navigator.clipboard.writeText = vi.fn().mockResolvedValue(undefined);
    const user = userEvent.setup();
    renderWithRouter(<CopyButton getSummary={() => "summary"} />);
    await user.click(screen.getByRole("button"));
    await waitFor(() =>
      expect(screen.getByText("Copied.")).toBeInTheDocument(),
    );
